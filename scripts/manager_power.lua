...
function getPCPowerAction(nodeAction, sSubRoll)
	...
	elseif rAction.type == "heal" then
		...
		for _,v in ipairs(aHealNodes) do
			...
			table.insert(rAction.clauses, { dice = aDice, stat = sAbility, modifier = nMod });
		end
...
function getPowerGroupRecord(rActor, nodePower, bNPCInnate)
	...
	if sActorType == "pc" then
		...
		for _,v in pairs(DB.getChildren(nodeActor, "traits")) do
			...
			if not bInnate and sTraitName == "spellcasting" then
				nodeTrait = v;
			elseif bInnate and sTraitName == "innate spellcasting" then
				nodeTrait = v;
			end
		end
		if nodeTrait then
	...
	if not aPowerGroup and nodePower then
		if sActorType == "pc" then
			ChatManager.SystemMessage(Interface.getString("power_error_spellgroupnotfound"));
		else
...
function evalAction(rActor, nodePower, rAction)
	...
	if (rAction.type == "cast") or (rAction.type == "attack") then
		...
		if (rAction.prof or 0) == 1 then
			rAction.modifier = rAction.modifier + ActorManager2.getAbilityBonus(rActor, "prf");
		end
	end
		...
		for _,vClause in ipairs(rAction.clauses) do
			if (vClause.stat or "") ~= "" then
				if vClause.stat == "base" then
					...
					if aPowerGroup then
						vClause.modifier = vClause.modifier + ActorManager2.getAbilityBonus(rActor, aPowerGroup.sStat);
						vClause.stat = "";
					end
				else
					vClause.modifier = vClause.modifier + ActorManager2.getAbilityBonus(rActor, vClause.stat);
					vClause.stat = "";
				end
			end
		end
				...
				if nDuration > 0 then
					...
					for _,v in ipairs(aActions) do
						if v.type == "effect" then
							if ((v.nDuration or 0) == 0) and (nDuration ~= 0) and (sEffect ~= "Prone") then
